(async function () {
    ...
    return new Promise((resolve) => {
      try {
        chrome.storage.local.get(defaults, (res) => {
          if (chrome.runtime.lastError || !res) resolve(defaults);
          else resolve(res);
        });
      } catch (e) {
        resolve(defaults);
  ...
  function cacheLoad() {
    return new Promise((resolve) => {
      try {
        chrome.storage.local.get({ [PCACHE_KEY]: [] }, (res) => {
          const saved = res && res[PCACHE_KEY];
  ...
  function cacheSave(entries) {
    try {
      chrome.storage.local.set({ [PCACHE_KEY]: entries }, () => {
        void chrome.runtime.lastError;
      });
    } catch (e) {}
  }
  ...
  function httpRequest(opts) {
    return new Promise((resolve) => {
      ...
      setTimeout(() => done({ status: 0, text: '', failure: 'timeout' }), timeout + 2000);
